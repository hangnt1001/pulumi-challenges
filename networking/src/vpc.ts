...
interface DbSecurityGroupArgs {
    /**
     * The VPC to which we allow traffic from.
     */
    Vpc: Vpc;
}
...
export class Vpc extends ComponentResource {
    vpc: aws.ec2.Vpc;
    privateZone: aws.route53.Zone | undefined;
    dhcpOptionSet: aws.ec2.VpcDhcpOptions | undefined;
    ...
    flowLogsGroup: aws.cloudwatch.LogGroup | undefined;
    flowLogsRole: aws.iam.Role | undefined;
    ...
    public createDBSecurityGroup(args: DbSecurityGroupArgs) {
        const VpcName = args.Vpc.name;

        const sg = new aws.ec2.SecurityGroup(`${this.name}-${VpcName}-sg`, {
            description: `Security Group that allows db traffic between ${this.name}`,
            egress: [
                { cidrBlocks: ["0.0.0.0/0"], fromPort: 0, protocol: "-1", toPort: 0 },
            ],
            ingress: [
                { cidrBlocks: [args.Vpc.vpc.cidrBlock], fromPort: 0, toPort: 0, protocol: "-1" },
            ],
            vpcId: this.vpcId(),
        }, { parent: this });
